...
const projectsData = [
  ...
  if (projectsGrid) {
    projectsData.forEach(proj => {
      ...
      card.className = 'project-showroom-card scroll-reveal-fade';
      card.setAttribute('data-id', proj.id);
      
      card.innerHTML = `
        <div class="project-sr-visual">
          <i class="${proj.icon}"></i>
          <span class="project-sr-difficulty ${difficultyClass}">${proj.difficulty}</span>
        </div>
        <div class="project-sr-content">
          <span class="project-sr-category">${proj.category}</span>
          <h3>${proj.title}</h3>
          <p class="project-sr-desc">${proj.desc}</p>
          <div class="project-sr-footer">
            <span class="project-sr-tech">${proj.tech}</span>
            <button class="project-details-btn" data-id="${proj.id}">
  ...
  if (certsGrid) {
    certsData.forEach(cert => {
      ...
      card.className = 'cert-dashboard-card scroll-reveal-fade';
